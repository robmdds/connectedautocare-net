...
export default function VSCQuoteResults() {
    ...
    useEffect(() => {
        const loadQuoteData = async () => {
            ...
            // Get quote data from localStorage (matching NewLanding.tsx)
            const savedQuote = localStorage.getItem('currentQuote');
            console.log("🔍 VSCQuoteResults: localStorage data:", savedQuote);

            if (savedQuote) {
                console.log("✅ VSCQuoteResults: Found quote data, processing...");
                const quoteData = JSON.parse(savedQuote);

                // Extract vehicle and customer info from the stored quote data
                const vehicleInfo: VehicleInfo = {
                    vin: quoteData.vehicle.vin,
                    year: quoteData.vehicle.year?.toString() || "Unknown",
                    make: quoteData.vehicle.make || "Unknown",
                    model: quoteData.vehicle.model || "Unknown",
                    mileage: quoteData.vehicle.mileage,
                    vehicleClass: quoteData.vehicle.vehicleClass || "Class A",
                ...
                const customerInfo: CustomerInfo = {
                    ...
                    zipcode: quoteData.customer.zipcode
                };
                ...
                console.log("❌ VSCQuoteResults: No quote data found in localStorage");
            }
